...
local function subscribeToModule(self)
  local cid = sessionBluetooth:SendRPC("GetInteriorVehicleData", { moduleType = "RADIO", subscribe = true})
  ...
  sessionBluetooth:ExpectResponse(cid)
end
...
local function isSubscribedBT(self)
  self.hmiConnection:SendNotification("RC.OnInteriorVehicleData", onIVDParams)
  sessionBluetooth:ExpectNotification("OnInteriorVehicleData", onIVDParams)
end

local function isSubscribedUSB(self)
  self.hmiConnection:SendNotification("RC.OnInteriorVehicleData", onIVDParams)
  sessionBluetooth:ExpectNotification("OnInteriorVehicleData")
  :Times(0)
  sessionUsb:ExpectNotification("OnInteriorVehicleData", onIVDParams)
  self:waitForAllEvents(2000)
end
...
runner.Step("SubscribeToModule", subscribeToModule)
runner.Step("Is Subscribed", isSubscribedBT)

runner.Step("Connect USB Device", connectUSBDevice)
runner.Step("Is still Subscribed", isSubscribedUSB)
