...
testCasesForPolicyTable:Precondition_updatePolicy_By_overwriting_preloaded_pt("files/ptu_general_default_keep_context_false.json")
...
function Test:TestStep_Verify_default_section()
  ...
  if(keep_context ~= false) then
    commonFunctions:printError("Error: keep_context is not false")
    test_fail = true
  end
