...
-- HMI sends <externalConsentStatus> to SDl via OnAppPermissionConsent ( all params present and within upper bounds, EntityStatus = "OFF")
...
function Test:TestStep_GetListofPermissions_AllParams_Upper()
  local RequestIdListOfPermissions = self.hmiConnection:SendRequest("SDL.GetListOfPermissions", {appID = self.applications[config.application1.registerAppInterfaceParams.appName]})
