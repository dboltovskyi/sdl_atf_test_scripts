---------------------------------------------------------------------------------------------
-- Requirement summary:
-- [Policies] Merging rules for "consumer_friendly_messages" section - does NOT exist in LocalPT and exists in PreloadedPT
-- --
-- Description:
-- Check of merging rules for "consumer_friendly_messages" section in case when section does not exist in LocalPT and exists in PreloadedPT
-- 1. Used preconditions
-- Delete files and policy table from previous ignition cycle if any
-- Start SDL with PreloadedPT json file with "preloaded_date" parameter and 1 language sections(en-us)
...
-- and 3 language sections(de-de, en-us, fr-fr) as child sections of "consumer_friendly_messages"
--
-- Expected result:
...
function Test:TestStep_VerifyNewLocalPT()
  ...
  local checks = {
    ...
    {
      query = 'select language_code from message where message_type_name = "Location"',
      expectedValues = {TESTED_DATA[1].key, TESTED_DATA[2].key, TESTED_DATA[3].key}
    },
    {
      query = 'select tts from message where message_type_name = "Location"',
      expectedValues = {TESTED_DATA[1].tts, TESTED_DATA[2].tts, TESTED_DATA[3].tts}
    },
    {
      query = 'select label from message where message_type_name = "Location"',
      expectedValues = {TESTED_DATA[1].label, TESTED_DATA[2].label, TESTED_DATA[3].label}
    }
  }
  if not self.checkLocalPT(checks) then
    self:FailTestCase("SDL has wrong values in LocalPT")
