...
--    - app: still activated
--    - embedded: published, not activated
---------------------------------------------------------------------------------------------------
...
local function publishEmbeddedService()
  ...
  common.getHMIConnection():ExpectResponse(cid, {
    result = {
      appServiceRecord = {
        serviceManifest = manifest,
        servicePublished = true,
        serviceActive = false -- embedded service is not activated automatically
      },
      code = 0,
  ...
  common.getMobileSession():ExpectNotification("OnSystemCapabilityUpdated", exp1)
  common.getHMIConnection():ExpectNotification("BasicCommunication.OnSystemCapabilityUpdated", exp1)
end

--[[ Scenario ]]
runner.Title("Preconditions")
runner.Step("Clean environment", common.preconditions)
runner.Step("Set EmbeddedServices parameter to EMPTY", common.setSDLIniParameter, { "EmbeddedServices", "" })
...
runner.Step("Publish Embedded Service MEDIA_not_active", publishEmbeddedService)
